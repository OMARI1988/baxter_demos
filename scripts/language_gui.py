...
class Learning:
    ...
    def load(self):
	pkl_file = open(self.directory+'data.pkl', 'rb')
	print ' - loading data..'
	...
	self.hyp = Test_HSV_Hypotheses(self.hyp,o_color,sentence)				# check to see if objects match any of the hypotheses

	if self.hyp['objects_hyp'][0] != []:
		self.POINTS_SPA = Update_SPA_histogram(o_qsr,self.POINTS_SPA)		# update the histogram of SPA
...
class simpleapp_tk(Tkinter.Tk):
	...
	print obj_r
	print sentence
	self.Learn.language_and_vision(sentence,obj_r)
