...
#--------------------------------------------------------------------------------------#
    def talker():
	global img,x,y,c,flag1,flag2,xyz
		...
		if flag1==1 and flag2==1:
			...
			for i in range(len(xyz2)-1):
				for j in range(i+1,len(xyz2)):
					...
					dis = np.sqrt(  (np.abs(xi-xj))**2  +  (np.abs(yi-yj))**2  +  (np.abs(zi-zj))**2  )
					dirr = direction_calc([xi,yi],[xj,yj])
					cv2.putText(img2,"distance [%s,%s] = %4.2f" % (i,j,dis), (10,480-20*counter), cv2.FONT_HERSHEY_SIMPLEX, .5, 0)
					counter+=1
					distance.append(dis)
					direction.append(dirr)
			pub.publish(ID,distance,direction,H,S,V)
